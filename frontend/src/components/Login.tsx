...
export default function Login({ onLogin }: { onLogin: (operator: string) => void }) {
  ...
  return (
    <div className="h-full w-full relative overflow-hidden bg-em-void">
      {/* Full RoboOps brand composition */}
      <img src="/login.png" alt="RoboOps Console" className="absolute inset-0 w-full h-full object-cover object-center" />
      <div className="absolute inset-0 bg-gradient-to-l from-em-void/70 via-transparent to-transparent" />

      {/* Sign-in panel over the image's right area */}
      <div className="relative z-10 h-full flex items-center justify-end pr-[5vw] pl-6">
        <form onSubmit={submit} className="w-[400px] max-w-[92vw] glass p-8 animate-riseIn">
